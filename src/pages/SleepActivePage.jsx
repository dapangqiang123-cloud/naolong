...
export default function SleepActivePage() {
  const { setCurrentView, wakeUpTime, playingMap, timerMap, toggleSound, setSoundTimer, clearSoundTimer, stopAllSounds } = useApp()
  ...
  const handleDismiss = () => {
    ...
    snoozeTimerRef.current = setTimeout(() => startAlarm(), 10 * 60 * 1000)
  }
  ...
  return (
    ...
    >
      ...
      {!isRinging && (
        <div className={`absolute top-12 left-0 w-full px-8 flex justify-between items-center transition-opacity duration-500 z-10 ${isDimmed ? 'opacity-0' : 'opacity-40 hover:opacity-100'}`}>
          ...
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-[#bbcf7c] text-sm" style={{fontVariationSettings:"'FILL' 1"}}>bedtime</span>
      ...
      <div className={`flex flex-col items-center text-center z-10 transition-opacity duration-1000 ${isDimmed && !isRinging ? 'opacity-20' : 'opacity-100'}`}>
        ...
        <span className="font-['Space_Grotesk'] font-light text-2xl tracking-[0.5em] text-[#909286] mt-2 opacity-60">{ampm}</span>

        {isRinging ? (
          <div className="flex flex-col items-center mt-12 gap-6">
            <div className="flex items-center gap-3 animate-pulse">
              <span className="material-symbols-outlined text-[#bbcf7c] text-2xl" style={{fontVariationSettings:"'FILL' 1"}}>alarm</span>
              <span className="font-['Space_Grotesk'] text-lg tracking-wider text-[#bbcf7c]">早安，该起床了</span>
            </div>
            {snoozeCount > 0 && (
              <p className="font-['Space_Grotesk'] text-xs text-[#909286] tracking-wider">已贪睡 {snoozeCount} 次</p>
            )}
            <p className="font-['Space_Grotesk'] text-xs text-[#45483e] tracking-widest uppercase">点击屏幕贪睡 10 分钟</p>
            <button
              ...
              style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#e5e2e1' }}>
              关闭闹钟
            </button>
          </div>
        ) : (
          <>
            <div className="mt-12 px-6 py-2 bg-white/5 backdrop-blur-md rounded-full flex items-center gap-3 border border-white/5">
              ...
              <span className="font-['Space_Grotesk'] text-xs tracking-wider text-[#c6c8ba]">唤醒时间 {wakeStr}</span>
            </div>
            <button
              ...
              <span className="material-symbols-outlined text-sm" style={{ color: '#bbcf7c', fontVariationSettings: "'FILL' 1" }}>
      ...
      {isDimmed && !isRinging && (
        <div className="absolute bottom-20 z-50 text-center">
          <p className="font-['Space_Grotesk'] text-[#45483e] text-xs tracking-widest uppercase">点击屏幕唤醒</p>
        </div>
      )}
                ...
                <div key={sound.id} className="rounded-xl p-4 transition-all"
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-['Space_Grotesk'] text-sm font-medium text-[#e5e2e1]">{sound.label}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      {isPlaying && (
                        <button
                          ...
                          style={{
                            background: remaining ? 'rgba(187,207,124,0.2)' : 'transparent',
                            color: remaining ? '#bbcf7c' : 'rgba(255,255,255,0.4)',
                            border: `1px solid ${remaining ? 'rgba(187,207,124,0.4)' : 'rgba(255,255,255,0.1)'}`,
                          }}>
                      ...
                      <button onClick={() => toggleSound(sound)}
                        ...
                        <span className="material-symbols-outlined text-base"
                              style={{ color: isPlaying ? '#0a0a0a' : 'rgba(255,255,255,0.5)', fontVariationSettings: "'FILL' 1" }}>
